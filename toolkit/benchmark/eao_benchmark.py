...
class EAOBenchmark(object):
    ...
    def eval(self, trackers):
        ...
        for tracker in trackers:
            eao = self._calculate_eao(tracker,self.tags)
            result[tracker] = eao
        return result
    ...
    def show_result(self, result, topk=10):
        ...
        if len(self.tags) == 1:
            ...
            for tracker_name, eao in tracker_eao:
                ...
                print(formatter.format(tracker_name, eao['all'])) #TODO: different from pysot, because only has tag 'all'
            print(bar)
        else:
    ...
    def _calculate_eao(self, tracker_name, tags):
        ...
        for video in self.dataset:
            gt_traj = video.gt_rects
            if tracker_name not in video.pred_bboxes:
                tracker_trajs = video.load_tracker(self.dataset.tracker_path, tracker_name, False)
            else:
                tracker_trajs = video.pred_bboxes[tracker_name]
            tracker_trajs = [tracker_trajs]  # TODO: different from pysot, understand it
        ...
        if len(tracker_trajs) == 0:
            print('Warning: some seqs in {} not found'.format(tags))
