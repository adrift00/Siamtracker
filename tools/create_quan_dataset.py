...
if __name__ == '__main__':
    ...
    for idx, data in enumerate(train_dataloader):
        ...
        base_model.forward(examplar_img,search_img)

        examplar_img = examplar_img.cpu().numpy()
        examplar_img = examplar_img.squeeze(0).transpose((1, 2, 0)).astype(np.float32)
        # np.set_printoptions(threshold=np.inf)
        # print(examplar_img[:,:,0])

        # examplar_path = os.path.join(quan_dataset_dir, 'examplar', 'examplar_{}.png'.format(idx + 1))
        # cv2.imwrite(examplar_path, examplar_img)
        np.save(os.path.join(quan_dataset_dir, 'examplar', 'examplar_{}.npy'.format(idx + 1)), examplar_img)

        search_img = search_img.cpu().numpy()
        search_img = search_img.squeeze(0).transpose((1, 2, 0)).astype(np.float32)
        # search_path = os.path.join(quan_dataset_dir, 'search', 'search_{}.png'.format(idx + 1))
        # cv2.imwrite(search_path, search_img)
        np.save(os.path.join(quan_dataset_dir, 'search', 'search_{}.npy'.format(idx + 1)), search_img)
        ...
        # search_img=np.ones((255,255,3))*255
        # search_img=torch.from_numpy(search_img.astype(np.float32))[None,:,:,:].permute(0,3,1,2).cuda()
